...
fun getCircleColor(@IntRange(from = RANDOM_COLOR_START_RANGE.toLong(), to = RANDOM_COLOR_END_RANGE.toLong())
    ...
    val colorId = MyApplication.getInstance().resources
            .getIdentifier(colorIdName, "color", MyApplication.getInstance().packageName)
...
fun getDrawable(@DrawableRes drawableId: Int): Drawable {
    return MyApplication.getInstance().resources.getDrawable(drawableId)
}

fun getColor(@ColorRes colorId: Int): Int {
    return MyApplication.getInstance().resources.getColor(colorId)
}

fun getDimen(@DimenRes dimenId: Int): Int {
    return MyApplication.getInstance().resources.getDimension(dimenId).toInt()
}